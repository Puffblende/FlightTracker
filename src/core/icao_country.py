"""ICAO 24-bit address → registration country.

Source: ICAO Annex 10, Volume III, addresses assigned by region/country.
Covers the major blocks; unallocated/unknown addresses return "".
"""
from __future__ import annotations
from bisect import bisect_right

# (start, end, country) – inclusive, sorted by start.
...
_STARTS = [b[0] for b in _BLOCKS]


def country_for_icao24(hex_str: str) -> str:
    """Return registration country name for an ICAO24 hex address, or ""."""
    if not hex_str:
        return ""
    try:
        addr = int(hex_str, 16)
    ...
    if i < 0:
        return ""
    start, end, country = _BLOCKS[i]
    return country if start <= addr <= end else ""
