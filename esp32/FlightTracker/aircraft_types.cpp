...
// Port of src/core/aircraft_types.py — ICAO type designator -> readable name.
// Keep in sync: regenerate by running this session's port script whenever
// the Python table changes.
struct TypeEntry { const char* code; const char* name; };
static const TypeEntry AIRCRAFT_TYPE_DB[] = {
    {"A109", "Leonardo AW109"},
    {"A124", "Antonov AN-124"},
    {"A139", "Leonardo AW139"},
    {"A140", "Antonov AN-140"},
...
static const int AIRCRAFT_TYPE_DB_COUNT = (int)(sizeof(AIRCRAFT_TYPE_DB) / sizeof(AIRCRAFT_TYPE_DB[0]));

const char* lookupAircraftType(const char* code) {
    if (!code || !code[0]) return "";
    char key[8] = {0};
    strncpy(key, code, sizeof(key) - 1);
    for (int i = 0; key[i]; i++) if (key[i] >= 'a' && key[i] <= 'z') key[i] -= 32;
    for (int i = 0; i < AIRCRAFT_TYPE_DB_COUNT; i++) {
        if (strcmp(AIRCRAFT_TYPE_DB[i].code, key) == 0)
            return AIRCRAFT_TYPE_DB[i].name;
    }
    return "";
}
